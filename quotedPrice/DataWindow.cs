...
namespace ProjectBudget
{
    public class DataWindow : ComboBox
    {
        ...
        public new Object DataSource
        {
            set
            {
                if (Connection == null && string.IsNullOrEmpty(SearchSql) && m_sDisplayField != String.Empty)
                {
        ...
        [Description("查询SQL"), Browsable(true), Category("N8")]
        public string SearchSql { get { return searchSql; } set { searchSql = value; } }

        [Description("查询连接"), Browsable(true), Category("N8")]
        ...
        {
            if (!RequeryDataSource())
            {
                DataView.RowFilter = GetRowFilterString(TextBox.Text);
            }
        }

        private bool RequeryDataSource()
        {
            if (Connection != null && !string.IsNullOrEmpty(SearchSql))
            {
                DbCommand comm = Connection.CreateCommand();
                comm.CommandText = SearchSql;
                ...
                {
                    dt.Columns.Add(new DataColumn(row[0].ToString(), row[0].GetType()));
                }
